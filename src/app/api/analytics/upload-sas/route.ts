...
export async function POST(request: Request) {
  try {
    ...
    const sasFile = formData.get("sasFile")

    if (!(sasFile instanceof File)) {
      return NextResponse.json(
        {
          error: "Missing sasFile upload.",
        },
        { status: 400 }
      )
    }

    const rawSasText = await sasFile.text()
...
`
    ...
    const rawMetricText = String(
      data.response ?? ""
    ).trim()

    const repairedMetricText =
      repairOllamaMetricText(rawMetricText)

    const parsedMetrics =
      parseOllamaMetricText(repairedMetricText)

    const canonicalMetrics = parsedMetrics.map(
      (metric) =>
        canonicalizeMetric({
    ...
    const artifact = canonicalMetricsToArtifact({
      ...
      suspectedFormat:
        "sas_logistic_raw_upload",

      question: {
        primary:
          "Which factors predict the target outcome?",
    ...
    return NextResponse.json({
      ...
      audit: {
        uploadedFileName: sasFile.name,

        parsedMetricCount:
          parsedMetrics.length,

        canonicalMetricCount:
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to process uploaded SAS output.",
      },
      { status: 500 }
    )
  }
