...
# Output:
# - Console metrics
# - Appends each run to metrics_artifact_store.json
# - Each run gets a unique PID
...
def extract_metrics(text: str) -> tuple[str, list[dict]]:
    source_type = detect_source_type(text)

    if source_type == "sas_logistic":
        return source_type, parse_sas_logistic(text)

    if source_type == "python_statsmodels_logit":
        return source_type, parse_python_statsmodels_logit(text)
...
def append_json_artifact(
    ...
    if artifact_path.exists():
        ...
        if existing_text:
            artifact_store = json.loads(existing_text)
        else:
            artifact_store = []
    else:
        artifact_store = []
...
def main() -> None:
    ...
    try:
        ...
        source_type, metrics = extract_metrics(text)
